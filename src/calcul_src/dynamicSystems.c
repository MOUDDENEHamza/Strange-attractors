...
void euler_t(double speed_t[], double position[], double parameter[], double *dt) {
    ...
    speed_t[1] = position[1] + ((-(parameter[0]) / (parameter[1])) * sin(position[0]) -
                                ((parameter[2]) / parameter[3]) * position[1]) * (*dt);
...
void aizawa_init(double speed[], double position[], double parameter[]) {
    ...
    speed[2] = parameter[2] + parameter[0] * position[2] - (pow(position[2], 3) / 3) -
               (pow(position[0], 2) + pow(position[1], 2)) * (1 + parameter[4] * position[2]) +
               parameter[5] * pow(position[0], 3) * position[2];
...
void aizawa_t(double speed_t[], double position[], double parameter[], double *dt) {
    ...
    speed_t[2] = position[2] + (parameter[2] + parameter[0] * position[2] - (pow(position[2], 3) / 3) -
                                (pow(position[0], 2) + pow(position[1], 2)) * (1 + parameter[4] * position[2]) +
                                parameter[5] * pow(position[0], 3) * position[2]) * (*dt);
...
void coullet_init(double speed[], double position[], double parameter[]) {
    ...
    speed[0] = position[1];
    speed[1] = position[2];
    speed[2] = parameter[0] * position[0] + parameter[1] * position[1] + parameter[2] * position[2] +
               parameter[3] * pow(position[0], 3);
...
void coullet_t(double speed_t[], double position[], double parameter[], double *dt) {
    ...
    speed_t[2] = position[2] + (parameter[0] * position[0] + parameter[1] * position[1] + parameter[2] * position[2] +
                                parameter[3] * pow(position[0], 3)) * (*dt);
...
void hadley_t(double speed_t[], double position[], double parameter[], double *dt) {
    ...
    speed_t[0] = position[0] +
                 (position[1] * position[0] - parameter[1] * position[2] * position[1] - position[0] + parameter[3]) *
                 (*dt);
    speed_t[1] = position[1] + (-pow(position[0], 2) - pow(position[2], 2) - parameter[0] * position[1] +
                                parameter[0] * parameter[2]) * (*dt);
    speed_t[2] =
            position[2] + (parameter[1] * position[0] * position[1] + position[0] * position[2] - position[2]) * (*dt);
